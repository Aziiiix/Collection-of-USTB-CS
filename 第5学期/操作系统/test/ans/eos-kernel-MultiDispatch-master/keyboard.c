	...
	)
{
	...
	case 1:
		...
		//
		if (ScanCode[0] <= 0x58) {
			// 判断是否按下了空格键，并且当前线程的优先级大于0并小于8，满足条件就将当前线程的优先级设为默认值,
			// 初始时间片的大小设为基础时间片的大小，剩余时间片的大小大小设为初始时间片的大小
			if (ScanCode[0] == 0x39  && PspCurrentThread->Priority > 0 && PspCurrentThread->Priority < 8) {		
