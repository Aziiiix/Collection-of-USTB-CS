...
PRIVATE
VOID
ConsoleCmdImprovepriorityThread(
	IN HANDLE StdHandle,
	IN PCSTR Arg
	);
	...
	)
{
	...
	for (;;){
		...
		if (0 == stricmp(Line, "ver")) {
			...
			ConsoleCmdImprovepriorityThread(StdHandle, Arg);
			continue;
...
--*/
{
	...
	for (i = 0; ; i++) {
		...
		fprintf(pThreadParameter->StdHandle, "Thread ID:%d, Priority:%d, InitTicks:%u, UsedTicks:%u, RemainderTicks:%u ",
			ObGetObjectId(PspCurrentThread),Priority, PspCurrentThread->InitTicks, PspCurrentThread->UsedTicks, PspCurrentThread->RemainderTicks);
...
功能描述：
	提升优先级由 ID 指定的线程。控制台命令“up threadid”。
...
--*/
{
	...
	if (EOS_SUCCESS(Status)) {
		...
		if (Thread->Priority > 0 && Thread->Priority < 8) {
			//
			if (Ready == Thread->State) {
				...
				Thread->Priority = 8;
				Thread->InitTicks =  TICKS_OF_TIME_SLICE;
				Thread->RemainderTicks =Thread->InitTicks;
				
				PspReadyThread(Thread);
				
				// 控制台线程由于等待键盘时间处于阻塞状态，会自动执行线程调度。
			
			
			} else {	//	线程处于阻塞态或运行态，直接提升优先级即可。
