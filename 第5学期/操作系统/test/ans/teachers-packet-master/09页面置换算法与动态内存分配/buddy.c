...
{ 
	...
	s = buddy(*p); 
	// 归并伙伴块
	while(s >= start && s < start + (*pav)[M].nodesize && s->bflag == 0 && s->bsize == (*p)->bsize)
	{	//伙伴块起始地址在有效范围内且伙伴块空闲 
		// 从链表上删除该结点 
		...
		else	// 链表上不止一个结点，则将该结点s删除 
		{
			...
			if((*pav)[s->bsize].first == s)	// s是链表的第一个结点 
				(*pav)[s->bsize].first = s->rlink; // 表头指向下一个结点 
		} 
		// 修改结点头部 
