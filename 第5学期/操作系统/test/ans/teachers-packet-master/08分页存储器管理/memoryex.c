...
int main(int argc, char* argv[])
{
	...
	ULONG UserPageCount = 0;	// 用户地址空间占用的物理页数量
	BOOL bIsUser;				// 用户地址空间标志
	
	ULONG PageTableCount = 0;	// 页表数量（包括页目录）
	...
	for(IndexOfDirEntry = 0; IndexOfDirEntry < PTE_PER_TABLE; IndexOfDirEntry++)
	{
		...
		//
		// 如果 PDE 标号小于 0x200，就是用户地址空间
		//
		if(IndexOfDirEntry < 0x200)
			bIsUser = TRUE;
		else
			bIsUser = FALSE;
			
		//
		// 每个有效的 PDE 都映射一个页表（包括复用为页表的页目录）
		//
		PageTableCount++;
	...
	//
	// 输出用户地址空间占用的物理页数，和物理内存数
	//
	OutputFormat = "\nUser Physical Page Total: %d\n";
	printf(OutputFormat, UserPageCount);

	OutputFormat = "User Physical Memory Total: %d\n\n";
	printf(OutputFormat, UserPageCount * PAGE_SIZE);

	
	//
	// 输出页表的数量（包括页目录）
	//
	OutputFormat = "\nPage Table Total (Include Page Directory): %d\n";
	printf(OutputFormat, PageTableCount);

	
	//
	// 软盘引导扇区在虚拟地址 0x80007C00 开始的 512 个字节，
	// 打印这部分内存。
	//
	ULONG ByteIndex;
	for(ByteIndex = 0; ByteIndex < 512; ByteIndex++)
	{
		...
		OutputFormat = "%X ";
		printf(OutputFormat, *((BYTE*)0x80007C00 + ByteIndex));
	}
